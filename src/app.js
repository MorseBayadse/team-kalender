...
// Gibt Map<dateStr, name> zurück für gesetzliche Feiertage RLP
...
// Besondere Anlässe RLP (kein gesetzl. Feiertag)
  ...
  for (let row = 0; row < 6; row++) {
    ...
    for (let col = 0; col < 7; col++) {
      ...
      html += `<div class="${cls}" onclick="selectDay('${ds}')">
        ...
        ${holLabelM}
        ${semEntry}
        ${dayEvs.slice(0,3).map(ev => `<div class="ev-pill" style="background:${ev.color||activeCalendarData?.color||'#5B5FEF'}" onclick="event.stopPropagation();openEventModal('${ev.id}','${ds}')">${roleLightHTML(ev)}${esc(ev.title)}</div>`).join('')}
...
function renderDayPanel(ds) {
  ...
  document.getElementById('dayEvents').innerHTML = dayEvs.length === 0
    ...
    : dayEvs.map(ev => `<div class="day-event-item" style="border-left-color:${ev.color||activeCalendarData?.color||'#5B5FEF'}" onclick="openEventModal('${ev.id}','${ds}')">
...
function setupYearZoom() {
  ...
  function checkZoom() {
    ...
    // Pinch-to-zoom ändert nicht die CSS-Pixel, nur den visuellen Scale
  ...
  for (let m = 0; m < 12; m++) {
    ...
    for (let row = 0; row < 6; row++) {
      ...
      for (let col = 0; col < 7; col++) {
        ...
        // Feiertag / Anlass Label (dot + bar + text für alle 3 Zoom-Stufen)
        let holEntry = '';
        if (holName) {
...
function renderWeekView() {
  ...
  const miniPill = (ev, colDs) => {
    const bg = ev.color||activeCalendarData?.color||'#5B5FEF';
    const t  = (ev.time||'').slice(0,5);
    return `<div class="week-mini-pill" style="background:${bg}" onclick="openEventModal('${ev.id}','${colDs||ev.date}')" title="${esc(ev.title)} ${t}">${roleLightHTML(ev)}<span class="wmp-t">${t}</span> ${esc(ev.title)}</div>`;
  };
  ...
  const mainCols = days.map(ds => {
    // Alle Termine, die an diesem Tag stattfinden (inkl. mehrtägige)
    const allDayEvs = events.filter(e => e.date === ds || (e.date_end && e.date <= ds && e.date_end >= ds));
    // Termine mit Uhrzeit: als zeitlich begrenzter Balken auf JEDEM Tag (nicht nur am Startdatum)
    const dayEvs = allDayEvs.filter(e => e.time);
    // Ganztägige Balken: nur für mehrtägige Termine OHNE Uhrzeit
    const alldayEvs = allDayEvs.filter(e => !e.time && e.date_end && e.date !== e.date_end);
    ...
    earlyHtml += `<div class="week-extra-cell${todayCls}${sunHolCls}">${earlyEvs.map(e => miniPill(e, ds)).join('')}</div>`;
    lateHtml  += `<div class="week-extra-cell${todayCls}${sunHolCls}">${lateEvs.map(e => miniPill(e, ds)).join('')}</div>`;
    ...
    const blocks = mainEvs.map(ev => {
      ...
      return `<div class="week-event-block" style="top:${top}px;height:${Math.max(dur,20)}px;left:4px;right:4px;background:${ev.color||activeCalendarData?.color||'#5B5FEF'}" onclick="openEventModal('${ev.id}','${ds}')">${roleLightHTML(ev)}${esc(ev.title)}</div>`;
    }).join('');
    ...
    const alldayBars = alldayEvs.map(ev => {
      const bg = ev.color||activeCalendarData?.color||'#5B5FEF';
      const isMulti = ev.date_end && ev.date !== ev.date_end;
      const isStart = ev.date === ds;
      const isEnd   = ev.date_end === ds || (!ev.date_end && isStart);
      const cls = isMulti ? ('week-multi-bar' + (isStart?' is-start':'') + (isEnd?' is-end':'')) : 'week-allday-bar';
      return `<div class="${cls}" style="background:${bg}" onclick="event.stopPropagation();openEventModal('${ev.id}','${ds}')" title="${esc(ev.title)}">${isStart?roleLightHTML(ev)+esc(ev.title):'&nbsp;'}</div>`;
    }).join('');
    const alldayWrap = alldayBars ? `<div class="week-allday-wrap">${alldayBars}</div>` : '';
    return `<div class="week-day-col${todayCls}${sunHolCls}${semWCls}" style="height:${HOURS*H}px">${alldayWrap}${lines}${blocks}${semWBlock}</div>`;
  }).join('');
...
function renderDayView() {
  ...
  const d      = new Date(ds+'T00:00:00');
  const events = getEvents().filter(e => e.date === ds || (e.date_end && e.date <= ds && e.date_end >= ds));
  const HOURS  = CAL_END_HOUR - CAL_START_HOUR; // 13 Slots (06..18)
  const H      = 60;
  document.getElementById('monthLabelText').textContent =
  ...
  // Ganztägige Balken: nur für Termine OHNE Uhrzeit (mit oder ohne date_end)
  const alldayDayEvs = events.filter(e => !e.time);
  const alldayRowEl = document.getElementById('dayAlldayItems');
  const alldayRow   = document.getElementById('dayAlldayRow');
  const alldayHTML = alldayDayEvs.map(ev => {
    const bg = ev.color||activeCalendarData?.color||'#5B5FEF';
    const isMulti = ev.date_end && ev.date !== ev.date_end;
    const label = isMulti ? `${esc(ev.title)} · ${ev.date} – ${ev.date_end}` : esc(ev.title);
    return `<div class="day-allday-item" style="background:${bg}" onclick="openEventModal('${ev.id}','${ds}')">${roleLightHTML(ev,'lg')}<span class="daa-title">${label}</span></div>`;
  }).join('');
  if (alldayRowEl) alldayRowEl.innerHTML = alldayHTML;
  if (alldayRow)   alldayRow.style.display = alldayDayEvs.length ? 'flex' : 'none';
  // Fallback: falls kein spezieller Row-Container existiert, oben im Events-Col zeigen
  if (!alldayRowEl && alldayDayEvs.length) {
    // wird weiter unten durch col.innerHTML zusammengesetzt (siehe Variable alldayInline)
  }
  const alldayInline = (!alldayRowEl && alldayDayEvs.length)
    ? `<div class="day-allday-inline">${alldayHTML}</div>` : '';
  // Timed-Termine (inkl. mehrtägige mit Uhrzeit – werden an jedem Tag als zeitlich begrenzter Balken gezeigt)
  ...
  const miniItem = ev => {
    const bg = ev.color||activeCalendarData?.color||'#5B5FEF';
    const t  = (ev.time||'').slice(0,5);
    return `<div class="day-mini-item" style="background:${bg}" onclick="openEventModal('${ev.id}','${ds}')">${roleLightHTML(ev,'lg')}<span class="dmi-t">${t}</span><span class="dmi-title">${esc(ev.title)}</span></div>`;
  };
  ...
  const blocks = mainEvs.map(ev => {
    ...
    const top = ((hh-CAL_START_HOUR)*60+mm)/60*H;
    const dur = ev.time_end ? (() => { const [eh,em]=(ev.time_end||'01:00').split(':').map(Number); const d=((eh*60+em)-(hh*60+mm))/60*H; return Math.min(d, (HOURS*H)-top); })() : H;
    return `<div class="day-event-timed" style="top:${top}px;height:${Math.max(dur,28)}px;left:8px;right:8px;background:${ev.color||activeCalendarData?.color||'#5B5FEF'}" onclick="openEventModal('${ev.id}','${ds}')">
  ...
  col.innerHTML = alldayInline + blocks + nowLine;
}
  ...
  document.getElementById('mineViewList').innerHTML = mineEvs.map(ev => {
    ...
    const badge = myRole
      ...
      : (roles.length === 0
          ...
          : '');
    return `<div class="my-event-row" style="border-left-color:${ev.color||activeCalendarData?.color||'#5B5FEF'}" onclick="openEventModal('${ev.id}','${ev.date}')">
...
window.openAddEvent = () => {
  ...
  // Wiederholungs-UI zurücksetzen
  const rep = document.getElementById('ev-repeat'); if (rep) { rep.value = 'none'; rep.disabled = false; }
  const bar2 = document.getElementById('ev-allday-bar'); if (bar2) bar2.checked = false;
  const ru = document.getElementById('ev-repeat-until'); if (ru) ru.value = '';
  document.querySelectorAll('#ev-repeat-weekdays input[type=checkbox]').forEach(c => c.checked=false);
  if (window.updateRepeatUI) window.updateRepeatUI();
...
// Helfer: Datum +/- N Tage als YYYY-MM-DD
function _shiftDate(ds, delta) {
  const d = new Date(ds+'T00:00:00');
  d.setDate(d.getDate()+delta);
  return toDateStr(d);
}
// Globaler Kontext: welcher Tag wurde angeklickt (für mehrtägige Termine)
window._editingTargetDay = null;
window.updateRepeatUI = () => {
  const sel = document.getElementById('ev-repeat').value;
  const wd = document.getElementById('ev-repeat-weekdays');
  if (wd) wd.style.display = sel === 'weekdays' ? 'block' : 'none';
};
window.openEventModal = (id, targetDay) => {
  const ev = getEvents().find(e => e.id === id);
  if (!ev) return;
  window._editingTargetDay = targetDay || ev.date;
  // Wiederholungs-UI beim Bearbeiten deaktivieren (greift nur beim Neuanlegen)
  const rep = document.getElementById('ev-repeat'); if (rep) { rep.value = 'none'; rep.disabled = true; }
  const bar3 = document.getElementById('ev-allday-bar');
  if (bar3) bar3.checked = !!(ev.date_end && ev.date_end !== ev.date);
  updateRepeatUI();
...
window.saveEvent = async () => {
  ...
  try {
    if (editingEventId) {
      const orig = getEvents().find(e => e.id === editingEventId);
      const isMulti = orig && orig.date_end && orig.date !== orig.date_end;
      const tDay = window._editingTargetDay || orig?.date;
      let mode = 'all';
      if (isMulti) {
        const ans = prompt('Mehrtägiger Termin – Änderung anwenden auf:\n  1 = Nur diesen Tag (' + tDay + ')\n  2 = Alle Tage\n\nBitte 1 oder 2 eingeben:', '2');
        if (ans === null) return; // abgebrochen
        mode = ans.trim() === '1' ? 'single' : 'all';
      }
      if (mode === 'all') {
        await DB.updateEvent(editingEventId, payload);
      } else {
        // Serie aufsplitten: Original auf Tage vor tDay beschränken, neuer Termin für tDay (mit neuen Daten), ggf. Rest-Serie nach tDay
        const origStart = orig.date;
        const origEnd   = orig.date_end;
        // 1) Neuer Termin nur für tDay mit neuen Payload-Daten (ohne date_end außer Nutzer hat geändert)
        const singlePayload = { ...payload, date: tDay, date_end: null };
        await DB.createEvent(currentUser.id, (orig.__calId || orig.calendar_id || activeCalendarId), singlePayload);
        // 2) Rest nach tDay als neue Serie (mit Original-Daten)
        if (tDay < origEnd) {
          const afterPayload = {
            title: orig.title, date: _shiftDate(tDay,1), date_end: origEnd,
            time: orig.time, time_end: orig.time_end,
            description: orig.description, location: orig.location, color: orig.color,
            roles: (orig.event_roles||[]).map(r => ({ name: r.name, assignedUserId: r.assigned_user_id })),
          };
          await DB.createEvent(currentUser.id, (orig.__calId || orig.calendar_id || activeCalendarId), afterPayload);
        }
        // 3) Original auf Tage vor tDay verkürzen oder löschen
        if (tDay > origStart) {
          await DB.updateEvent(editingEventId, { date_end: _shiftDate(tDay,-1) });
        } else {
      ...
      // Neuer Termin – Wiederholung / Datumsbereich
      const repType = document.getElementById('ev-repeat')?.value || 'none';
      const alldayBar = document.getElementById('ev-allday-bar')?.checked;
      const dates = [];
      if (repType === 'none') {
        if (payload.date_end && payload.date_end !== payload.date && !alldayBar) {
          // Datum Von–Bis ohne Ganztägig: einzelne Events pro Tag
          const start = new Date(date+'T00:00:00');
          const end   = new Date(payload.date_end+'T00:00:00');
          for (let d = new Date(start); d <= end; d.setDate(d.getDate()+1)) {
            dates.push(toDateStr(d));
          }
        } else {
          // Einzeltermin ODER Mehrtages-Balken (ganztägig)
          await DB.createEvent(currentUser.id, activeCalendarId, payload);
        }
      } else if (repType === 'weekly') {
        // Wöchentlich – dauerhaft (50 Jahre in die Zukunft = 2600 Wochen)
        const start = new Date(date+'T00:00:00');
        for (let w = 0; w < 2600; w++) {
          const d = new Date(start); d.setDate(d.getDate() + w*7);
          dates.push(toDateStr(d));
        }
      } else if (repType === 'weekdays') {
        const until = document.getElementById('ev-repeat-until').value;
        if (!until) { showToast('Bitte Enddatum der Serie wählen.', 'error'); return; }
        const weekdays = Array.from(document.querySelectorAll('#ev-repeat-weekdays input[type=checkbox]:checked')).map(c => parseInt(c.value,10));
        if (!weekdays.length) { showToast('Bitte mindestens einen Wochentag wählen.', 'error'); return; }
        const start = new Date(date+'T00:00:00');
        const end   = new Date(until+'T00:00:00');
        for (let d = new Date(start); d <= end; d.setDate(d.getDate()+1)) {
          if (weekdays.includes(d.getDay())) dates.push(toDateStr(d));
        }
      }
      // Einzelne Events (ohne Balken-Logik) anlegen – date_end weglassen
      if (dates.length) {
        const base = { ...payload, date_end: null };
        for (const ds of dates) {
          await DB.createEvent(currentUser.id, activeCalendarId, { ...base, date: ds });
        }
        showToast(`${dates.length} Termine erstellt!`);
      } else if (repType === 'none') {
...
window.deleteEvent = async () => {
  ...
  try {
    if (isMulti) {
      const ans = prompt('Mehrtägiger Termin – Löschen:\n  1 = Nur diesen Tag (' + tDay + ')\n  2 = Gesamten Termin (alle Tage)\n\nBitte 1 oder 2 eingeben:', '2');
      if (ans === null) return;
      if (ans.trim() === '1') {
        // Nur diesen Tag entfernen
        const origStart = orig.date, origEnd = orig.date_end;
        if (tDay === origStart) {
          await DB.updateEvent(editingEventId, { date: _shiftDate(tDay,1) });
        } else if (tDay === origEnd) {
          await DB.updateEvent(editingEventId, { date_end: _shiftDate(tDay,-1) });
        } else {
          await DB.updateEvent(editingEventId, { date_end: _shiftDate(tDay,-1) });
          await DB.createEvent(currentUser.id, (orig.__calId || orig.calendar_id || activeCalendarId), {
            title: orig.title, date: _shiftDate(tDay,1), date_end: origEnd,
            time: orig.time, time_end: orig.time_end,
            description: orig.description, location: orig.location, color: orig.color,
            roles: (orig.event_roles||[]).map(r => ({ name: r.name, assignedUserId: r.assigned_user_id })),
        ...
        if (!confirm('Gesamten Termin wirklich löschen?')) return;
        await DB.deleteEvent(editingEventId);
      }
    } else {
...
function renderSettings(tab) {
  ...
  if (tab === 'general') {
    ...
    body.innerHTML = `
      ...
      <div class="settings-card" id="pendingInvitesCard" style="display:none">
        <div class="settings-card-title">📧 Offene E-Mail-Einladungen</div>
        <div id="pendingInvitesList"></div>
      </div>`;
    // Offene E-Mail-Einladungen laden (asynchron)
    if (canManage) loadPendingInvites(settingsCalId);
  } else if (tab === 'sharing') {
    renderSharingTab();
  }
}
...
async function inviteByEmail(email) {
  try {
    // 1) Nutzer per E-Mail in der Datenbank suchen
    const profile = await DB.findUserByEmail(email);

    if (profile) {
      // Nutzer existiert → direkt als Mitglied einladen
      if (profile.id === currentUser.id) {
        showToast('Du kannst dich nicht selbst einladen.', 'error');
        return;
      }
      // Prüfen ob schon Mitglied
      const existing = activeCalendarData?.members?.find(m => m.user_id === profile.id);
      if (existing) {
        showToast('Dieser Nutzer ist bereits Mitglied oder eingeladen.', 'error');
        return;
      }
      ...
      showToast(`✅ ${profile.firstname} ${profile.lastname} wurde eingeladen!`);
    } else {
      // Nutzer nicht registriert → Pending-Einladung + Mailto-Link
      const calName = activeCalendarData?.name ?? 'Team-Kalender';
      try {
        const invitation = await DB.createPendingInvitation(email, activeCalendarId, currentUser.id);
        const inviteUrl = `${window.location.origin}${window.location.pathname}?invite=${invitation.token}`;
        showInviteMailDialog(email, calName, inviteUrl);
        // Einladungsliste aktualisieren
        renderSettings('members');
        showToast('📧 Einladung erstellt! E-Mail wird vorbereitet…');
      } catch (err) {
        if (err.message?.includes('duplicate') || err.message?.includes('unique')) {
          showToast('Diese E-Mail wurde bereits zu diesem Kalender eingeladen.', 'error');
        } else {
          throw err;
        }
      }
...
/** Mailto-Dialog anzeigen mit vorbereiteter Einladungs-Mail */
function showInviteMailDialog(email, calendarName, inviteUrl) {
  ...
  // Dialog anzeigen
  let dialog = document.getElementById('inviteMailDialog');
  if (!dialog) {
    dialog = document.createElement('div');
    dialog.id = 'inviteMailDialog';
  ...
  dialog.innerHTML = `
    <div class="modal" style="max-width:480px">
      <div class="modal-header">
        <h2>📧 Einladung versenden</h2>
        <button onclick="closeInviteMailDialog()" class="btn-close">✕</button>
      </div>
      <div class="modal-form" style="padding:20px">
        <p style="margin:0 0 12px;color:var(--text-secondary)">
          <strong>${esc(email)}</strong> ist noch nicht registriert. Eine Einladung wurde erstellt.
        </p>
        <p style="margin:0 0 16px;color:var(--text-secondary);font-size:14px">
          Sende die Einladung per E-Mail, damit sich die Person registrieren und dem Kalender
          <strong>"${esc(calendarName)}"</strong> beitreten kann.
        </p>
        <div style="background:var(--surface-raised,#f3f4f6);border-radius:8px;padding:12px;margin-bottom:16px;word-break:break-all;font-size:12px;color:var(--text-secondary)">
          ${esc(decodeURIComponent(inviteUrl.replace(/%20/g, ' ')))}
        </div>
        <div style="display:flex;gap:10px;flex-wrap:wrap">
          <a href="${mailtoLink}" class="btn-primary" style="text-decoration:none;text-align:center;flex:1;min-width:150px"
             onclick="setTimeout(()=>closeInviteMailDialog(),500)">
            📧 E-Mail öffnen
          </a>
          <button onclick="copyInviteLink('${inviteUrl.replace(/'/g, "\\'")}')" class="btn-secondary" style="flex:1;min-width:120px">
            📋 Link kopieren
          </button>
        </div>
      </div>
...
window.closeInviteMailDialog = () => {
  const d = document.getElementById('inviteMailDialog');
  if (d) d.classList.remove('open');
};

window.copyInviteLink = async (url) => {
  try {
...
window.cancelPendingInvite = async (invId) => {
  if (!confirm('Einladung zurückziehen?')) return;
  try {
    await DB.deletePendingInvitation(invId);
    renderSettings('members');
...
async function loadPendingInvites(calendarId) {
  try {
    const invites = await DB.getPendingInvitations(calendarId);
    ...
    list.innerHTML = invites.map(inv => {
      ...
      return `<div class="member-row">
        <div class="member-ava" style="background:var(--border);color:var(--text-muted);font-size:14px">📧</div>
        <div class="member-info">
          <div class="name">${esc(inv.email)}</div>
          <div class="email">Eingeladen am ${date} · Noch nicht registriert</div>
        </div>
        <button class="btn-danger" style="padding:5px 10px;font-size:11px" onclick="cancelPendingInvite('${inv.id}')">✕</button>
      </div>`;
    }).join('');
  } catch (_) {}
}
...
// ── Einladungs-Token aus URL verarbeiten ─────────────────────

/** Prüft ob ein ?invite=TOKEN in der URL ist und zeigt Einladungsinfo an */
async function checkInviteToken() {
  const params = new URLSearchParams(window.location.search);
  const token = params.get('invite');
  if (!token) return;

  // Token merken für nach der Registrierung
  ...
  // Einladungsinfo laden und anzeigen
  try {
    const inv = await DB.getInvitationByToken(token);
    if (inv) {
      showInviteBanner(inv);
    } else {
      showMsg('Diese Einladung ist ungültig oder wurde bereits verwendet.', 'error');
    }
  } catch (_) {}
}

/** Banner über dem Auth-Formular anzeigen mit Einladungsinfo */
function showInviteBanner(inv) {
  ...
  if (!banner) {
    ...
    if (authCard) authCard.parentNode.insertBefore(banner, authCard);
  }
  banner.style.cssText = 'background:linear-gradient(135deg,#EEF2FF,#E0E7FF);border:1.5px solid #6C5CE7;border-radius:12px;padding:16px 20px;margin-bottom:16px;max-width:400px;width:100%;text-align:center;';
  banner.innerHTML = `
    <div style="font-size:24px;margin-bottom:8px">📅</div>
    <div style="font-weight:700;color:#1F2937;font-size:15px;margin-bottom:4px">
      Einladung zum Kalender
    </div>
    <div style="color:#6C5CE7;font-weight:600;font-size:16px;margin-bottom:8px">
      ${esc(inv.calendar_name)}
    </div>
    <div style="color:#6B7280;font-size:13px">
      Eingeladen von <strong>${esc(inv.inviter_name)}</strong>
    </div>
...
// Einladungs-Token beim Laden prüfen
checkInviteToken();
