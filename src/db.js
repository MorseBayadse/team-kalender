...
// ── EINLADUNGEN (auch für nicht-registrierte Nutzer) ────────
...
/** Nutzer per E-Mail suchen (in auth.users via RPC). */
export async function findUserByEmail(email) {
  const { data, error } = await supabase.rpc('find_user_by_email', { p_email: email });
...
/** Einladung an eine E-Mail-Adresse erstellen (für nicht-registrierte Nutzer). */
export async function createPendingInvitation(email, calendarId, invitedBy, role = 'member') {
  const token = randomInviteToken();
  const { data, error } = await supabase
    .from('pending_invitations')
    .insert({ email: email.toLowerCase(), calendar_id: calendarId, invited_by: invitedBy, role, token })
    .select()
    .single();
  if (error) throw error;
  return data;
...
/** Alle offenen Einladungen eines Kalenders laden. */
export async function getPendingInvitations(calendarId) {
  const { data, error } = await supabase
    .from('pending_invitations')
...
/** Offene Einladung löschen / zurückziehen. */
export async function deletePendingInvitation(invitationId) {
...
/** Einladung per Token abrufen (für Landing Page, ohne Auth nötig). */
